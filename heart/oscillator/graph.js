...
class Graph {
    ...
    legend(ctx, xCol, yCol, row1D=true, row2D=true, xtext="AP", ytext="AP/dt") {
        ...
        if (row1D) {
            ...
            ctx.fillStyle = this.axisColor;
            ctx.fillText(xtext, row1[0] - (xtext.length * 12 + 15), row1[1] + 5);
            ctx.beginPath();
        ...
        if (row2D) {
            ...
            ctx.fillText(ytext, row2[0] - (ytext.length * 10 + 15), row2[1] + 5);
            ctx.beginPath();
...
class CompGraphs {
    ...
    draw(ctx) {
        ...
        this.graph.legend(ctx, this.xColor, this.yColor);
        ...
        this.hgraph.legend(ctx, this.xColor, this.yColor);
...
class IntroGraph {
    ...
    draw(ctx) {
        ...
        this.graph.axis(ctx);
        this.graph.drawTitle(ctx);
        this.graph.legend(ctx, this.xColor, this.yColor, true, false, "Action potential", "");
